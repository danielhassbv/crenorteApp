...
import { Rota } from '../app/models/rota.model';
...
export class RotasService {
  ...
  /** Tenta header:true (colunas), senão cai para header:false (matriz). */
        ...
        complete: (result) => {
          try {
            ...
            if (temLatLng) {
              ...
              // Fallback: matriz (arrays puros por linha)
              Papa.parse(file, {
                header: false,
                skipEmptyLines: true,
                complete: (raw) => {
  ...
  /** Matriz de metas (header:false): SUPERVISOR/ANALISTA/META + colunas por localidade. */
  private parseMatrizArrays(rows: any[][]): Rota[] {
    const norm = (v: any) => String(v ?? '').trim();

    // 1) Linha do cabeçalho com SUPERVISOR + ANALISTA
    ...
    // 2) Descobrir nomes de localidades por coluna (3..N) procurando linhas posteriores
    const localidadesMap = new Map<number, string>(); // col -> nome
    for (let i = idxHeader + 8; i < rows.length; i++) {
      ...
      if (localidadesMap.size >= 5 && i > (idxHeader + 10)) break;
    }

    // Se por algum motivo não achou nomes, ainda assim percorre colunas usando "Col X"
    const colStart = 3;
    ...
    // 3) Linhas com analistas/metas (aprox. logo após o header)
    const rotas: Rota[] = [];
    for (let i = idxHeader + 3; i < Math.min(idxHeader + 12, rows.length); i++) {
      ...
      for (let col = colStart; col < colEnd; col++) {
        const localidade = localidadesMap.get(col) ?? `Coluna ${col}`;
        const meta = this.parseNumero(row[col]);
        if (Number.isFinite(meta) && meta > 0) {
          const { bairro, municipio } = this.splitLocalidade(localidade);
          rotas.push({
            id: `${analista || supervisor}-${localidade}-${i}-${col}`,
            data: '',
            assessor: analista || supervisor || '—',
            municipio,
            bairro,
            latitude: NaN,    // geocodifica depois
            longitude: NaN,   // geocodifica depois
            status: 'planejada',
            peso: meta
          });
        }
